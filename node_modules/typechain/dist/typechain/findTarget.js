...
function findTarget(config) {
    ...
    const possiblePaths = [
        ...
        files_1.ensureAbsPath(target),
    ];
    const moduleInfo = lodash_1.default(possiblePaths).compact().map(modules_1.tryRequire).compact().first();
    if (!moduleInfo || !moduleInfo.module.default) {
        throw new Error(`Couldn't find ${config.target}. Tried loading: ${lodash_1.compact(possiblePaths).join(', ')}.\nPerhaps you forgot to install @typechain/${target}?`);
    }
