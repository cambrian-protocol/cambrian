...
describe("It should all work", function () {
  ...
  it("Should execute single-solver Proposal with two outcomes", async function () {
    ...
    solverConfigs.forEach((config, index) => {
      if (index == 0) {
        let p = this.SolverFactory.createSolver(
          ...
          .then((tx) => tx.wait())
          .then((rc) => {
            console.log(rc.events[0].data);
            solvers.push(rc.events[0].data);
          });
        promises.push(p);
      } else {
        let p = new ethers.Contract(
          solvers[index - 1],
          SOLVER_ABI,
          ethers.provider
        )
    ...
    return Promise.all(promises).then(console.log(solvers));
