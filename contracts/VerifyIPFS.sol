...
library VerifyIPFS {
    ...
    /// @param content The content of the IPFS object
    /// @return The IPFS hash in base 58
    function generateHash(bytes memory content)
        internal
        pure
    ...
    function verifyHash(bytes memory content, bytes memory hash)
        internal
        pure
        returns (bool)
    {
        return equal(generateHash(content), hash);
    }
