...
contract BinaryCondition {
    ...
    modifier protected() {
        ...
        require(msg.sender == avatar, "CtfTreasury: protected operation");
        _;
    }

    modifier checkOracle() {
        require(oracle != address(0), "CtfTreasury: oracle not set");
        _;
    }

    modifier onlyOracle() {
        require(msg.sender == oracle, "CtfTreasury: caller must be oracle");
        _;
    }
    ...
    function initialize(
        ...
        require(_avatar != address(0), "CtfTreasury: avatar cannot be null");
        require(
            _conditionalTokens != ConditionalTokens(address(0)),
            "CtfTreasury: conditionalTokens cannot be null"
        );
        avatar = _avatar;
        conditionalTokens = _conditionalTokens;
        fee = _fee;
     ...
     */
    function setFee(uint256 _newFee) external protected {
        fee = _newFee;
        emit FeeSet(fee);
    }
     ...
     */
    function setOracle(address _oracle) external protected {
        require(_oracle != address(0), "CtfTreasury: oracle cannot be null");
        oracle = _oracle;
    ...
    function createBinaryCondition(
        ...
        for (uint256 i = 0; i < _binaryIndexSets.length; i++) {
    ...
    function redeemPositions(
        ...
        if (_success == true) {
            ...
            if (_parentCollectionId == bytes32(0)) {
                uint256 balanceMinusFee = _balance -
                    ((_balance * (uint256(PPM))) * (fee)) /
                    (PPM100);
                uint256 feeAmount = ((_balance * (uint256(PPM))) * (fee)) /
                    (PPM100);
                require(
