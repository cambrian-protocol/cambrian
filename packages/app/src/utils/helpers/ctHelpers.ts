 ...
 * @return {BN} BN
 */
var toBN = function (number: number | string) {
    try {
...
export function getCollectionId(conditionId: string, indexSet: number) {
    // const initHash = soliditySha3(
    //     { t: 'bytes32', v: conditionId },
    //     { t: 'uint', v: indexSet }
    // )

    const initHash = ethers.utils.solidityKeccak256(
        ['bytes32', 'uint'],
...
export function combineCollectionIds(collectionIds: string[]) {
...
export function getPositionId(collateralToken: string, collectionId: string) {
    // return soliditySha3(
